...
//a function for the read action
unsigned int App::read(Wallet &wallet, cxxopts::ParseResult args)
{
  ...
  else
  {
    ...
    try
    {
      category = wallet.getCategory(args["category"].as<std::string>());
    }
    catch (...)
    {
    ...
    if (!args.count("item"))
    {
      ...
      {
        std::cout << category.str() << std::endl;
      }
    }
    else
    {
      ...
      try
      {
        item = category.getItem(args["item"].as<std::string>());
      }
      catch (...)
      {
      ...
      {
        try{
          std::cout << item.getEntry(args["entry"].as<std::string>()) << std::endl;
        }catch(...){
