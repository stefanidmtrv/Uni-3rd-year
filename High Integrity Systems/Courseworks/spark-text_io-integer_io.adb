...
is
   ...
   begin
      case File.Sort is
         ...
         when Std_In =>
            ...
            File.Status := Success;
         when A_File =>
            Ada_Integer_IO.Get (File.File, Int, Width);
            Item := (Status => Success, Item => Int);
            File.Status := Success;
      end case;
   exception
   ...
   begin
      case File.Sort is
         when Std_In =>
            File.Status := Mode_Error;
         when Std_Out =>
            Ada_Integer_IO.Put (Ada.Text_IO.Standard_Output,
                                Item, Width, Base);
            File.Status := Success;
         when Std_Error =>
            Ada_Integer_IO.Put (Ada.Text_IO.Standard_Error,
                                Item, Width, Base);
            File.Status := Success;
         when A_File =>
   ...
   procedure Put (Item  : in Num;
                  ...
                  Base  : in Number_Base := Default_Base) is
   begin
      Ada_Integer_IO.Put (Ada.Text_IO.Standard_Output, Item, Width, Base);
